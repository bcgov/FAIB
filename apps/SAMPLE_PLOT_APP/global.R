...
getSpatialQueryIaian<-function(sql){
  conn<-dbConnect(dbDriver("PostgreSQL"), host='inserthost', dbname = 'insert DB', port='5432' ,user='appuser' ,password='wonttell')
...
sp_samplePoints <- getSpatialQueryIaian("SELECT 
                                        objectid,samp_id,sampletype,species_class,bgc_zone,tsa_desc,species_class,
beclabel,project_design,meas_dt,no_meas,spc_label_live,stemsha_liv,baha_liv,wsvha_liv,tot_stand_age,geom, samp_sts
from sample_plots_unique_final where geom is not null")
